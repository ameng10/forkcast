...
Deno.test("Principle: Register, Login, Logout Flow", async () => {
  ...
  try {
    ...
    assertEquals(
      (logoutResult as { user: ID }).user,
      userId,
      "Logout should return the user ID.",
    );
    ...
    assertEquals(
      isNowLoggedOut[0].loggedIn,
      false,
      "User should now be marked as logged out.",
    );
  } finally {
    await client.close();
  }
...
Deno.test("Action: register enforces unique username", async () => {
  ...
  try {
    ...
    const firstRegister = await authConcept.register({
      username: usernameA,
      password: passwordA,
    });
    assertNotEquals(
    ...
    const secondRegister = await authConcept.register({
      username: usernameA,
      password: "anotherpassword",
    });
    assertEquals(
      "error" in secondRegister,
      true,
    ...
    assertEquals(
      ...
      "Should return correct error message for duplicate username.",
    );
  } finally {
    await client.close();
  }
...
Deno.test("Action: login enforces requirements", async () => {
  ...
  try {
    ...
    const badPasswordResult = await authConcept.login({
      username: usernameA,
      password: "wrongpassword",
    });
    assertEquals(
      "error" in badPasswordResult,
      true,
      "Login with bad password should fail.",
    );
    assertEquals(
      (badPasswordResult as { error: string }).error,
      "Invalid username or password.",
    );

    // Test case: login with non-existent username.
    console.log("  Step: Attempt login with non-existent username.");
    const badUsernameResult = await authConcept.login({
      username: "nonexistent",
      password: passwordA,
    });
    assertEquals(
      "error" in badUsernameResult,
      true,
      "Login with bad username should fail.",
    );
    assertEquals(
      (badUsernameResult as { error: string }).error,
      "Invalid username or password.",
    );
    ...
    const alreadyLoggedInResult = await authConcept.login({
      username: usernameA,
      password: passwordA,
    });
    assertEquals(
      "error" in alreadyLoggedInResult,
      true,
      "Second login attempt should fail.",
    );
    assertEquals(
      (alreadyLoggedInResult as { error: string }).error,
...
Deno.test("Action: logout enforces requirements", async () => {
  ...
  try {
    ...
    const notLoggedInResult = await authConcept.logout({
      user: registeredUserId,
    });
    assertEquals(
      "error" in notLoggedInResult,
      true,
      "Logout should fail for a user not logged in.",
    );
    assertEquals(
      (notLoggedInResult as { error: string }).error,
      "User is not logged in.",
    );

    // Test case: attempt to log out a non-existent user.
    console.log("  Step: Attempt to log out a non-existent user.");
    const nonExistentUserResult = await authConcept.logout({
      user: nonExistentUserId,
    });
    assertEquals(
      "error" in nonExistentUserResult,
      true,
      "Logout should fail for a non-existent user.",
    );
    assertEquals(
...
Deno.test("Query: _getUserByUsername handles existing and non-existing users", async () => {
  ...
  try {
    ...
    const foundUser = await authConcept._getUserByUsername({
      username: usernameA,
    });
    ...
    const notFoundUser = await authConcept._getUserByUsername({
      username: "nonexistent",
    });
