  ...
  try {
    ...
    await t.step("Phase 1: Ingest observations", async () => {
      ...
      await concept.ingest({
        owner: userAlice,
        at: getDate(-9),
        signals: [signalFried, signalDairy],
        metric: metricEnergyLevel,
        value: 2,
      });
      await concept.ingest({
      ...
      await concept.ingest({
        owner: userAlice,
        at: getDate(-4),
        signals: [signalFried, signalLateNight],
        metric: metricEnergyLevel,
        value: 1,
      }); // Very negative combo
      await concept.ingest({
      ...
      const aliceObservations = await concept._getObservationsForUser(
        userAlice,
      );
      assertEquals(
        aliceObservations.length,
        9,
        "Alice should have 9 observations.",
      );
    ...
    await t.step("Phase 2: Analyze observations", async () => {
      ...
      const analyzeResult = await concept.analyze({
        owner: userAlice,
        window: 12,
      });
      assertEquals(
        ...
        false,
        `Analysis for Alice failed: ${JSON.stringify(analyzeResult)}`,
      );
      ...
      assertEquals(
        aliceInsights.length,
        6,
        "Alice should have 6 distinct insights after analysis.",
      );

      // Verify specific insight calculations
      const friedInsight = aliceInsights.find((i) =>
        i.signals.length === 1 && i.signals[0] === signalFried
      );
      ...
      assertEquals(
        friedInsight!.effect.toFixed(1),
        (-2.5).toFixed(1),
        "Fried insight effect should be -2.5.",
      );
      assertEquals(
        friedInsight!.confidence.toFixed(1),
        (0.2).toFixed(1),
        "Fried insight confidence should be 0.2.",
      );

      const healthyInsight = aliceInsights.find((i) =>
        i.signals.length === 1 && i.signals[0] === signalHealthy
      );
      ...
      const emptySignalsInsight = aliceInsights.find((i) =>
        i.signals.length === 0
      );
      assertExists(
        emptySignalsInsight,
        "Insight for [] (no signals) should exist.",
      );
      // Values for []: 5. Avg = 5. Effect = 5 - 5 = 0. Confidence = 1/10 = 0.1
      assertEquals(
        emptySignalsInsight!.effect,
        0,
        "Effect for [] should be 0.",
      );
      assertEquals(
        emptySignalsInsight!.confidence.toFixed(1),
        (0.1).toFixed(1),
      );
    });
    ...
    await t.step(
      "Phase 3: Summarize observations and insights into a report",
      async () => {
        const summarizeResult = await concept.summarize({
          owner: userAlice,
          period: "week",
        });
        assertEquals(
          "error" in summarizeResult,
          false,
          `Summarize for Alice failed: ${JSON.stringify(summarizeResult)}`,
        );
        ...
        assertExists(
          generatedReport,
          "Generated report should be retrievable.",
        );
        assertEquals(
          generatedReport!.owner,
          userAlice,
          "Report owner should be Alice.",
        );
        assertEquals(
          generatedReport!.period,
          "week",
          "Report period should be 'week'.",
        );
        assertExists(
          generatedReport!.generatedAt,
          "Report should have a generatedAt timestamp.",
        );
        assertEquals(
          generatedReport!._id,
          reportId,
          "Generated report _id should match the returned report ID.",
        );
        ...
        assertEquals(
          generatedReport!.topHelpful?.length,
          1,
          "There should be 1 top helpful signal.",
        );
        assertArrayIncludes(generatedReport!.topHelpful as ID[], [
          signalHealthy,
        ]);
        ...
        assertEquals(
          generatedReport!.topHarmful?.length,
          3,
          "There should be 3 top harmful signals.",
        );
        assertArrayIncludes(generatedReport!.topHarmful as ID[], [
          signalLateNight,
          signalFried,
          signalDairy,
        ]);
        ...
        // Verify metricTrends
        assertExists(
          generatedReport!.metricTrends,
          "Metric trends should exist.",
        );
        assertEquals(
          generatedReport!.metricTrends?.length,
          1,
          "There should be 1 metric trend for energy level.",
        );
        assertEquals(
          generatedReport!.metricTrends![0].metric,
          metricEnergyLevel,
        );
        // Average value for metricEnergyLevel for Alice over the period (all 9 observations):
        // (3+2+4+5+2+4+1+9+8) / 9 = 38 / 9 = 4.222...
        assertEquals(
          generatedReport!.metricTrends![0].value.toFixed(3),
          (4.222).toFixed(3),
          "Metric trend value should be the average of observations in period.",
        );
      },
    );
  } finally {
...
Deno.test("Action: ingest - Adds observation correctly and handles empty signals", async (t) => {
  ...
  try {
    ...
    await t.step("Ingest with signals", async () => {
      const result = await concept.ingest({
        owner: userAlice,
        at: atTime,
        signals: [signalDairy],
        metric: metricMood,
    ...
    await t.step("Ingest with empty signals", async () => {
      ...
      const emptyObs = observations.find((o) =>
        o.metric === metricProductivity
      );
      assertExists(emptyObs);
      assertEquals(
        emptyObs!.signals.length,
        0,
        "Signals array should be empty.",
      );
  ...
  try {
    ...
    await t.step("Requires: No observations in window", async () => {
      ...
      assertEquals(
        "error" in result1,
        true,
        "Analyze should fail if no observations found.",
      );
      assertEquals(
        (result1 as { error: string }).error,
        "No observations found for owner within the specified window.",
      );
      ...
      assertEquals(
        "error" in result2,
        true,
        "Analyze should fail if observations are outside the window.",
      );
      assertEquals(
    ...
    await t.step(
      "Effects: Correctly computes and upserts insights with multiple observations",
      async () => {
        ...
        const analyzeResult1 = await concept.analyze({
          owner: userAlice,
          window: 3,
        });
        assertEquals(
          "error" in analyzeResult1,
          false,
          `Analyze failed: ${JSON.stringify(analyzeResult1)}`,
        );

        let insights = await concept._getInsightsForUser(userAlice);
        assertEquals(insights.length, 2, "Should have two insights.");

        const friedInsight = insights.find((i) =>
          i.signals.includes(signalFried) && i.signals.length === 1
        ); // Specific to [signalFried]
        ...
        const analyzeResult2 = await concept.analyze({
          owner: userAlice,
          window: 3,
        });
        assertEquals(
          "error" in analyzeResult2,
          false,
          `Analyze failed: ${JSON.stringify(analyzeResult2)}`,
        );
        ...
        assertEquals(
          insights.length,
          2,
          "Still should have two insights after upsert.",
        );

        const updatedFriedInsight = insights.find((i) =>
          i.signals.includes(signalFried) && i.signals.length === 1
        );
        assertExists(updatedFriedInsight);
        assertEquals(
          updatedFriedInsight!.effect.toFixed(2),
          ((3 + 1 + 5) / 3 - 5).toFixed(2),
        ); // (3-5 = -2)
        assertEquals(
          updatedFriedInsight!.confidence.toFixed(1),
          (3 / 10).toFixed(1),
        );

        const updatedDairyInsight = insights.find((i) =>
          i.signals.includes(signalDairy) && i.signals.length === 1
        );
        assertExists(updatedDairyInsight);
        assertEquals(updatedDairyInsight!.effect, (6 + 7) / 2 - 5); // (6.5-5 = 1.5)
        assertEquals(
          updatedDairyInsight!.confidence.toFixed(1),
          (2 / 10).toFixed(1),
        );
      },
    );
    ...
    await t.step(
      "Effects: Handles observations with no signals array correctly",
      async () => {
        ...
        assertEquals(
          "error" in analyzeResult,
          false,
          `Analyze failed: ${JSON.stringify(analyzeResult)}`,
        );
        ...
        assertEquals(
          insights.length,
          1,
          "Should have one insight for empty signals.",
        );

        const emptySignalsInsight = insights.find((i) =>
          i.signals.length === 0
        );
        ...
        assertEquals(
          emptySignalsInsight!.effect,
          (7 + 8) / 2 - 5,
          "Effect for empty signals should be correct (2.5).",
        );
        assertEquals(
          emptySignalsInsight!.confidence.toFixed(1),
          (0.2).toFixed(1),
...
Deno.test("Action: summarize - Requirements and effects on report content", async (t) => {
  ...
  try {
    await t.step("Requires: No observations in period", async () => {
      const result1 = await concept.summarize({
        owner: userAlice,
        period: "week",
      });
      assertEquals(
        "error" in result1,
        true,
        "Summarize should fail if no observations found in period.",
      );
      assertEquals(
        (result1 as { error: string }).error,
        "No observations found for owner within the specified period.",
      );
    });
    ...
    await t.step("Requires: Unsupported period string", async () => {
      ...
      const result2 = await concept.summarize({
        owner: userAlice,
        period: "month",
      });
      assertEquals(
        "error" in result2,
        true,
        "Summarize should fail for unsupported period.",
      );
      assertEquals(
        (result2 as { error: string }).error,
        "Unsupported period: month. Only 'week' is supported for now.",
      );
    });

    await t.step(
      "Effects: Generates report with topHelpful/topHarmful and metricTrends",
      async () => {
    ...
    await t.step(
      "Effects: Handles cases where no signals are helpful/harmful",
      async () => {
        ...
        assertEquals(
          "error" in analyzeResult,
          false,
          `Analyze for Charlie failed: ${JSON.stringify(analyzeResult)}`,
        );
        ...
        assertEquals(
          "error" in summarizeResult,
          false,
          `Summarize for Charlie failed: ${JSON.stringify(summarizeResult)}`,
        );
        const { report: reportId } = summarizeResult as { report: ID };
        const generatedReport = await concept._getReport(reportId);
        ...
        assertEquals(
          generatedReport!.topHelpful,
          [],
          "topHelpful should be an empty array if no signals have positive effect.",
        );
        assertEquals(
          generatedReport!.topHarmful,
          [],
          "topHarmful should be an empty array if no signals have negative effect.",
        );
        assertExists(generatedReport!.metricTrends);
        assertEquals(generatedReport!.metricTrends?.length, 1);
      },
...
Deno.test("Action: deactivate - Requires insight to exist and correct requester", async (t) => {
  ...
  try {
    await t.step("Setup: Ingest and analyze to create an insight", async () => {
      ...
      assertEquals(
        initialInsights[0].active,
        true,
        "Insight should be active initially.",
      );
    });
    ...
    await t.step("Requires: Unauthorized requester", async () => {
      ...
      assertEquals(
        "error" in unauthorizedResult,
        true,
        "Deactivation by unauthorized user should fail.",
      );
      assertEquals(
        (unauthorizedResult as { error: string }).error,
    ...
    await t.step("Requires: Non-existent insight", async () => {
      ...
      assertEquals(
        ...
        true,
        "Deactivation of non-existent insight should fail.",
      );
      assertEquals(
        (nonExistentResult as { error: string }).error,
        "No matching insight found to deactivate.",
      );
    });
  } finally {
    await client.close();
...
Deno.test("Action: deactivate - Successfully deactivates insight and handles re-deactivation", async (t) => {
  ...
  try {
    ...
    await t.step("Effects: Successfully deactivates an insight", async () => {
      ...
      assertEquals(
        "error" in deactivateResult,
        false,
        `Deactivation failed: ${JSON.stringify(deactivateResult)}`,
      );

      const insightsAfterDeactivation = await concept._getInsightsForUser(
        userAlice,
      );
      assertEquals(insightsAfterDeactivation.length, 1);
      assertEquals(
        insightsAfterDeactivation[0].active,
        false,
        "Insight should be deactivated.",
      );
    });

    await t.step(
      "Edge Case: Attempt to deactivate an already inactive insight",
      async () => {
        ...
        assertEquals(
          "error" in reDeactivateResult,
          true,
          "Re-deactivating an already inactive insight should fail to find an active match.",
        );
        assertEquals(
          (reDeactivateResult as { error: string }).error,
          "Insight is already inactive.",
        );
      },
    );
  } finally {
...
Deno.test("Queries: Retrieval functions work correctly", async (t) => {
  ...
  try {
    ...
    await t.step("_getObservationsForUser", async () => {
      ...
      assertEquals(
        aliceObservations.some((o) => o.signals.includes(signalFried)),
        true,
      );
      assertEquals(
        aliceObservations.some((o) => o.signals.includes(signalDairy)),
        true,
      );
    ...
    await t.step("_getInsightsForUser", async () => {
      ...
      const analyzeAliceResult = await concept.analyze({
        owner: userAlice,
        window: 3,
      });
      assertEquals(
        "error" in analyzeAliceResult,
        false,
        `Analyze for Alice failed: ${JSON.stringify(analyzeAliceResult)}`,
      );

      const analyzeBobResult = await concept.analyze({
        owner: userBob,
        window: 3,
      });
      assertEquals(
        "error" in analyzeBobResult,
        false,
        `Analyze for Bob failed: ${JSON.stringify(analyzeBobResult)}`,
      );
    ...
    await t.step("_getReport", async () => {
      ...
      const analyzeResult = await concept.analyze({
        owner: userAlice,
        window: 2,
      }); // Analyze to ensure insights are present for summarization logic
      assertEquals(
        "error" in analyzeResult,
        false,
        `Analyze failed: ${JSON.stringify(analyzeResult)}`,
      ...
      assertEquals(
        nonExistentReport,
        null,
        "Should return null for a non-existent report ID.",
      );
    });
  } finally {
    await client.close();
