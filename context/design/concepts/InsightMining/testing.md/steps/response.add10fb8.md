...
content_id: add10fb8f977d7316b3bd772bf4f5c5e755741b26995c6d0122f62850a4ba2de
---
...
export default class InsightMiningConcept {
    ...
    async summarize(
    ...
    ): Promise<{ report: ID } | { error: string }> { // --- MODIFIED RETURN TYPE HERE ---
        const now = new Date();
        ...
        const topHarmful: Signal[] = sortedSignalsByEffect
            .filter(s => s.avgEffect < 0)
            .slice(-TOP_N_SIGNALS) // Get the N most harmful (lowest avgEffect)
            .map(s => s.signal);
        ...
        return { report: report._id }; // --- MODIFIED RETURN VALUE HERE ---
    }
    ...
    async deactivate(
        ...
        // Requires: insight exists for (owner, signals, metric)
        // Also check if it's already active to return a more precise error
        const result = await this.insights.updateOne(
            { _id: insightId, owner: owner, active: true }, // Find the specific active insight
            { $set: { active: false } }, // Set its 'active' flag to false
        );

        if (result.matchedCount === 0) {
            // Could differentiate between not found vs. already inactive
            const existingInsight = await this.insights.findOne({ _id: insightId, owner: owner });
            if (existingInsight && !existingInsight.active) {
