...
content_id: e7878a9dabc4215421a4a32e7352e824219df53da38b15c19ae3464564b957fd
---
...
Deno.test("Principle: System ingests, analyzes, and summarizes observations", async (t) => {
  ...
  try {
    ...
    await t.step("Phase 2: Analyze observations", async () => {
      ...
      // Values: 3, 2, 2. Avg = 7/3 = 2.33. Effect = 2.33 - 5 = -2.67. Confidence = 3/10 = 0.3
      ...
      // Values: 9, 8. Avg = 17/2 = 8.5. Effect = 8.5 - 5 = 3.5. Confidence = 2/10 = 0.2
      ...
      // Values: 5. Avg = 5. Effect = 5 - 5 = 0. Confidence = 1/10 = 0.1
    ...
    await t.step("Phase 3: Summarize observations and insights into a report", async () => {
      ...
      const { report } = summarizeResult as { report: ID };
      assertExists(report);

      const generatedReport = await concept._getReport(report._id);
      ...
      // [fried, dairy]: -3
      // [dairy]: -1
      // []: 0 (this insight won't contribute to signal-based topHelpful/Harmful)
      // [fried, late_night]: -4
      // [healthy]: 3.5

      // Signal average effects:
      // signalFried: Avg((-2.67) + (-3) + (-4)) / 3 = -9.67 / 3 = -3.22
      // signalDairy: Avg((-3) + (-1)) / 2 = -2
      // signalLateNight: -4
      // signalHealthy: 3.5
      ...
      // Top Harmful: signalLateNight (-4), signalFried (-3.22), signalDairy (-2)
      ...
      // Check for inclusion as order of signals with same effect might not be guaranteed by sort
...
Deno.test("Action: analyze - Requires observations in window and calculates correctly", async (t) => {
  ...
  try {
    await t.step("Requires: No observations in window", async () => {
      ...
      const result2 = await concept.analyze({ owner: userAlice, window: 1 });
...
Deno.test("Action: deactivate - Requires insight to exist and correct requester", async (t) => {
  ...
  try {
    ...
    await t.step("Requires: Non-existent insight", async () => {
      ...
      assertEquals((nonExistentResult as { error: string }).error, "No matching active insight found to deactivate.");
    });
  } finally {
    await client.close();
  }
...
Deno.test("Action: deactivate - Successfully deactivates insight and handles re-deactivation", async (t) => {
  ...
  try {
    ...
    await t.step("Edge Case: Attempt to deactivate an already inactive insight", async () => {
      ...
      // The implementation correctly returns an error because `updateOne` with `{ active: false }`
      // would not match an already inactive document.
      assertEquals("error" in reDeactivateResult, true, "Re-deactivating an already inactive insight should fail to find a match.");
      assertEquals((reDeactivateResult as { error: string }).error, "No matching active insight found to deactivate.");
    });
  } finally {
    await client.close();
  }
