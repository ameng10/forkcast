...
content_id: e48128817d39837c027cbe1fdb966658a2619a0ff04cd7658227408882e67a9c
---
...
Deno.test("Principle: System ingests, analyzes, and summarizes observations", async (t) => {
  ...
  try {
    ...
    await t.step("Phase 2: Analyze observations", async () => {
      ...
      // Values for [fried]: 3, 2, 2, 1 (from fried, late_night, but only single 'fried' signal used for this insight)
      // The `createInsightKey` makes insight unique for each combo.
      // Values for unique signalFried insight: 3 (from obs 1), 2 (from obs 2), 2 (from obs 5) -> avg (3+2+2)/3 = 2.33
      // No, `createInsightKey` uses ALL signals. So [signalFried] is one insight.
      // [signalFried, signalDairy] is another. [signalFried, signalLateNight] is another.
      // For the insight where signals is exactly [signalFried]: Observations are {signals: [signalFried]}
      // Observations with only signalFried:
      // 1. { signals: [signalFried], value: 3 }
      // 2. { signals: [signalFried], value: 2 }
      // 3. { signals: [signalFried], value: 2 }
      // Values for this specific insight key: 3, 2, 2. Avg = 7/3 = 2.33. Effect = 2.33 - 5 = -2.67. Confidence = 3/10 = 0.3
    ...
    await t.step("Phase 3: Summarize observations and insights into a report", async () => {
      ...
      // Insights (Effect, Signals):
      // [signalFried]: -2.67
      // [signalDairy, signalFried]: -3.00 (from 2 observations of [fried, dairy], values 2) => avg 2, effect -3
      // [signalDairy]: -1.00
      // []: 0
      // [signalFried, signalLateNight]: -4.00
      // [signalHealthy]: 3.50

      // Signal average effects calculation (from insights):
      // signalFried: (-2.67 + -3 + -4) / 3 = -9.67 / 3 = -3.22 (approx)
      // signalDairy: (-3 + -1) / 2 = -2
      // signalLateNight: -4 / 1 = -4
      // signalHealthy: 3.5 / 1 = 3.5
      ...
      // signalLateNight (-4), signalFried (~-3.22), signalDairy (-2)
      ...
      assertArrayIncludes(generatedReport!.topHarmful as ID[], [signalFried, signalDairy, signalLateNight]); // Order can vary, so use assertArrayIncludes
...
Deno.test("Action: ingest - Adds observation correctly and handles empty signals", async (t) => {
  ...
  try {
    ...
    await t.step("Ingest with signals", async () => {
      ...
      assertEquals(observations[0].metric, metricMood); // Corrected comparison
