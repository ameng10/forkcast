...
content_id: cd5483999fb944855000edca1757403191f9274ba7826e56169afc724c3caa6e
---
...
const createInsightKey = (owner: User, signals: Signal[], metric: Metric): ID => {
    // Sort signals to ensure a consistent key regardless of input order
...
export default class InsightMiningConcept {
    ...
    async summarize(
        ...
        const topHarmful: Signal[] = sortedSignalsByEffect
            .filter(s => s.avgEffect < 0)
            .sort((a,b) => a.avgEffect - b.avgEffect) // Sort ascending for most harmful
            .slice(0, TOP_N_SIGNALS) // Get the N most harmful
            .map(s => s.signal);
        ...
        const reportId = freshID(); // Generate ID for the report
        const report: ReportDoc = {
            _id: reportId, // Assign the generated ID to the document
            owner,
            period,
            generatedAt: now,
        ...
        // Effects: Return the ID of the newly created report
    ...
    async deactivate(
        ...
        // Requires: insight exists for (owner, signals, metric) and is currently active
        // Find the specific active insight to ensure it's not already inactive
        const existingInsight = await this.insights.findOne({ _id: insightId, owner: owner });
        ...
        const result = await this.insights.updateOne(
            { _id: insightId, owner: owner },
            { $set: { active: false } },
        );

        if (result.matchedCount === 0) {
            // This case should ideally not be hit after the above explicit checks,
            // but remains as a final safeguard or for concurrent updates.
            return { error: "No matching active insight found to deactivate." };
        }
