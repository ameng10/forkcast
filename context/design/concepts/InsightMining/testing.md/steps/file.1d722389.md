...
content_id: 1d722389f193b18b135fb809d8bb54ce7a60e667a2dc60d135ec2df04f58444d
---
...
Deno.test("Principle: System ingests, analyzes, and summarizes observations", async (t) => {
  ...
  try {
    ...
    await t.step("Phase 3: Summarize observations and insights into a report", async () => {
      ...
      assertEquals(generatedReport!._id, reportId, "Generated report _id should match the returned report ID."); // Compare with reportId
      ...
      // Signal average effects (simplified calculation for verification):
      // signalFried: Avg((-2.67) + (-3) + (-4)) / 3 = -9.67 / 3 = -3.22
      // signalDairy: Avg((-3) + (-1)) / 2 = -2
      // signalLateNight: -4 (only one insight contributes this signal)
      // signalHealthy: 3.5 (only one insight contributes this signal)
...
Deno.test("Action: ingest - Adds observation correctly and handles empty signals", async (t) => {
  ...
  try {
    ...
    await t.step("Ingest with signals", async () => {
      ...
      assertEquals(observations[0].metric, metricMood); // This is the correct way
