...
content_id: 24a42d4b88ada61fb98fed36b039a05196be079af83e2d499e0b67cf26d2ecb5
---
...
export default class InsightMiningConcept {
     ...
     * @returns {{report: ReportDoc} | {error: string}} - The generated report document or an error.
     *
     * Requires: At least one observation must exist for the owner within the specified period.
     * Effects: A new report document is created in the 'reports' collection and returned.
     *
    ...
    async summarize(
        ...
        const topHarmful: Signal[] = sortedSignalsByEffect
            .filter(s => s.avgEffect < 0)
            .slice(-TOP_N_SIGNALS) // Get the most harmful (lowest avgEffect)
            .map(s => s.signal);
    ...
    async deactivate(
        ...
        const result = await this.insights.updateOne(
            { _id: insightId, owner: owner }, // Find the specific insight
