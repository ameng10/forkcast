---
timestamp: 'Thu Oct 16 2025 18:32:22 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_183222.9996c515.md]]'
content_id: 9f4979a057d9d193e51f46bb082fb62e51e5b30fba64a63b4fcf64f7a984067c
---
...
* A set of Meals with:
  * owner: User (represented by `ownerId` in the database for loose coupling)
  * at: Date
  * items: Set(FoodItem) (represented as an `Array<FoodItem>` in the database, where `FoodItem` is an embedded object)
  * notes: String (optional)
  * status: {active, deleted}

**actions**

* `submit(owner: User, at: Date, items: FoodItem[], notes?: string): Promise<Meal>`
  * **requires:** `owner` is a valid `User` object (with an `id`) and `items` is nonempty.
  * **effects:** Create a new `Meal` document in the `Meals` MongoDB collection with `status` set to `ACTIVE`. Returns the newly created `Meal` object.
* `edit(caller: User, mealId: string, items?: FoodItem[], notes?: string): Promise<void>`
  * **requires:** `mealId` refers to an existing `Meal` document. `caller` is the owner of the meal (by `id`). The `Meal`'s `status` is `ACTIVE`. If `items` is provided, it must be nonempty.
  * **effects:** Update the provided fields (`items`, `notes`) of the `Meal` document in MongoDB.
* `delete(caller: User, mealId: string): Promise<void>`
  * **requires:** `mealId` refers to an existing `Meal` document. `caller` is the owner of the meal (by `id`). The `Meal`'s `status` is `ACTIVE`.
  * **effects:** Set the `status` of the `Meal` document to `DELETED` in MongoDB.
