---
timestamp: 'Fri Nov 07 2025 12:38:39 GMT-0500 (Eastern Standard Time)'
parent: '[[../20251107_123839.3c28325e.md]]'
content_id: e90e45b7e875ae31c3ab9b9170f57affafbb59ceae5fe7a52e07c90669fb4091
---
...
export default class UserAuthenticationConcept {
  ...
  constructor(private readonly db: Db) {
    ...
    // Ensure username is unique at the database level to enforce the spec
  ...
  async register(
    ...
    // We rely on the unique index in MongoDB, but checking first provides a cleaner flow.
    try {
      ...
      // Catch potential duplicate key error from the database index during a race condition
      if (e.code === 11000) {
        return { error: "Username already exists" };
      }
  ...
  async login(
    ...
    // for both "not found" and "wrong password" cases.
    if (!user) {
      // We still hash a dummy password to make the timing similar to a failed login.
  ...
  async _getUserByUsername(
    ...
    // As per specification, queries must return an array. An empty array means not found.
    return [];
  }
}
```
