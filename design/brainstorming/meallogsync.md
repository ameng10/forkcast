...
[@userauthenticationsync](../brainstorming/userauthenticationsync.md)


# implement: Create the meallog syncronizations using the concepts above. The only included/public actions are the following:
...
There should be a sync where a submit of a meal should also get the meals of the user to show.
...
/**
 * Catches any request to a protected /meals/* path that has an invalid session token.
 * This provides a consistent authorization failure response.
 */
export const MealAuthFailure: Sync = ({ request, session, error }) => ({
  // Use a regex to match any path starting with /meals/
  when: actions([Requesting.request, { path: /^\/meals\//, session }, { request }]),
  where: async (frames) => {
    // A query to Sessioning._getUser with an invalid session will produce a frame
    // containing an 'error' binding. This sync specifically looks for that error.
    return await frames.query(Sessioning._getUser, { session }, { error });
  },
  then: actions([Requesting.respond, { request, error: "Invalid session or authorization failure." }]),
});
...
export const SubmitMealRequest: Sync = ({ request, session, user, at, items, notes }) => ({
  when: actions([
    ...
    { path: "/meals/submit", session, at, items, notes },
    { request },
  ]),
  where: async (frames) => {
 ...
 */
export const SubmitMealResponseSuccess: Sync = ({ request, user, meal, meals, mealDoc }) => ({
  when: actions(
    [Requesting.request, { path: "/meals/submit" }, { request }],
  ...
  where: async (frames) => {
    ...
    const framesWithMeals = await frames.query(MealLog._getMealsByOwner, { owner: user }, { meal: mealDoc });
...
export const EditMealRequest: Sync = ({ request, session, user, meal, at, items, notes }) => ({
  when: actions([
    Requesting.request,
    { path: "/meals/edit", session, meal, at, items, notes },
    { request },
  ]),
...
export const EditMealResponseSuccess: Sync = ({ request }) => ({
  when: actions(
    [Requesting.request, { path: "/meals/edit" }, { request }],
    [MealLog.edit, {}, {}],
  ),
...
export const DeleteMealRequest: Sync = ({ request, session, user, meal }) => ({
  when: actions([
    Requesting.request,
    { path: "/meals/delete", session, meal },
    { request },
  ]),
  where: (frames) => frames.query(Sessioning._getUser, { session }, { user }),
...
export const DeleteMealResponseSuccess: Sync = ({ request }) => ({
  when: actions(
    [Requesting.request, { path: "/meals/delete" }, { request }],
    [MealLog.delete, {}, {}],
  ),
 ...
 */
export const ListMeals: Sync = ({ request, session, user, mealDoc, meals }) => ({
  when: actions([
    Requesting.request,
    { path: "/meals/list", session },
    { request },
  ]),
  where: async (frames) => {
    ...
    const mealFrames = await framesWithUser.query(MealLog._getMealsByOwner, { owner: user }, { meal: mealDoc });
 ...
 */
export const GetMealSuccess: Sync = ({ request, session, user, meal, mealDoc }) => ({
  when: actions([Requesting.request, { path: "/meals/get", session, meal }, { request }]),
  where: async (frames) => {
    frames = await frames.query(Sessioning._getUser, { session }, { user });
    if (frames.length === 0) return new Frames(); // Invalid session

    frames = await frames.query(MealLog._getMealById, { meal }, { meal: mealDoc });
    ...
    return frames.filter($ => $[mealDoc]?.owner === $[user]);
  },
 ...
 */
export const GetMealNotFound: Sync = ({ request, session, user, meal, mealDoc, error }) => ({
  when: actions([Requesting.request, { path: "/meals/get", session, meal }, { request }]),
  where: async (frames) => {
    const framesWithUser = await frames.query(Sessioning._getUser, { session }, { user });
    if (framesWithUser.length === 0) return new Frames(); // Let auth failure sync handle this

    const mealFrames = await frames.query(MealLog._getMealById, { meal }, { meal: mealDoc });
 ...
 */
export const GetMealPermissionDenied: Sync = ({ request, session, user, meal, mealDoc, error }) => ({
  when: actions([Requesting.request, { path: "/meals/get", session, meal }, { request }]),
  where: async (frames) => {
    const framesWithUser = await frames.query(Sessioning._getUser, { session }, { user });
    if (framesWithUser.length === 0) return new Frames(); // Let auth failure sync handle this
    ...
    const authorizedFrames = mealFrames.filter($ => $[mealDoc]?.owner === $[user]);
    if (authorizedFrames.length === 0) {
      // The meal exists, but the user is not the owner. Add the error.
      return framesWithUser.map($ => ({ ...$, [error]: "Permission denied" }));
    }
